...
export const deleteAllOrderFromUser = async (req, res) => {
    try {
        const { userId } = req.body;

        const user = await User.findOne({ _id: userId, role: "user" }).populate(
            "orders"
        );
        if (!user) {
            return res.status(404).json({
        ...
        return res.status(200).json({
            status: "success",
            message: `Order deleted successfully from user: ${user.name}`,
        });
    } catch (error) {
        return res
            .status(500)
